...
func doAuth() error {
	...
	err = copyFilefromTemplate("templates/views/login.jet", cel.RootPath+"templates/views/login.jet")
	if err != nil {
		exitGracefully(err)
	}

	err = copyFilefromTemplate("templates/views/forgot.jet", cel.RootPath+"templates/views/forgot.jet")
	if err != nil {
		exitGracefully(err)
	}

	err = copyFilefromTemplate("templates/views/reset-password.jet", cel.RootPath+"templates/views/reset-password.jet")
	if err != nil {
		exitGracefully(err)
	}

	err = copyFilefromTemplate("templates/views/login.jet", cel.RootPath+"/mailer/login.jet")
	if err != nil {
		exitGracefully(err)
	}
