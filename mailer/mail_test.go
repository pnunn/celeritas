...
func TestMail_SendUsingChan(t *testing.T) {
	...
	res := <-mailer.Results
	if res.Error != nil {
		t.Error(errors.New("failed to send via channel"))
	}
...
func TestMail_send(t *testing.T) {
	...
	err = mailer.Send(msg)
	if err == nil {
		t.Error(errors.New("didn't get an error for invalid credentials"))
	}
...
func TestMail_ChooseAPI(t *testing.T) {
	...
	if err == nil {
		t.Error(errors.New("didn't get an error from choose an api, but should have"))
	}
}